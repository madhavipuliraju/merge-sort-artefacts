 ...
 function handlers(){ 
  document.getElementById("array1").onclick = function() { array1(1); };
  document.getElementById("array2").onclick = function() { array2(2); };
  ...
  function draw() {
    ...
    if (merge_artefact.merged && merge_artefact.current3==10){
      context.font = "17px Arial";
      context.fillText("Arrays are now merged!", canvas.width / 2 - 100, canvas.height-10 );
      context.font = "15px Arial";
    }
    else if (merge_artefact.merged) {
      context.font = "17px Arial";
      context.fillText("Wrong step, Try again!", canvas.width / 2 - 100, canvas.height-10 );
      context.font = "15px Arial";
    }
    context.stroke();
  }
  ...
  function array1(arrayno) {
    if ((merge_artefact.list1[merge_artefact.current1] <= merge_artefact.list2[merge_artefact.current2]) && (arrayno == 1)){
      merge_artefact.merged = false;
      step();
    }
    else if (merge_artefact.current1 < merge_artefact.list1.length && merge_artefact.current2 == merge_artefact.list2.length){
      merge_artefact.merged = false;
      step();
    }
    else{
  ...
  function array2(arrayno) {
    if ((merge_artefact.list1[merge_artefact.current1] > merge_artefact.list2[merge_artefact.current2]) && (arrayno == 2)){
      merge_artefact.merged = false;
      step();
    }
    else if (merge_artefact.current2 < merge_artefact.list2.length && merge_artefact.current1 == merge_artefact.list1.length){
      merge_artefact.merged = false;
      step();
    }
    else{
